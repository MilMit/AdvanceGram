...
public class Translator {
    ...
    public static void showTranslationTargetSelector(Context context, boolean isKeyboard, Runnable callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        PopupHelper.show(names, LocaleController.getString("TranslationLanguage", R.string.TranslationLanguage), targetLanguages.indexOf(isKeyboard ? AdvanceGramConfig.translationKeyboardTarget : AdvanceGramConfig.translationTarget), context, i -> {
            if (isKeyboard) {
                AdvanceGramConfig.setTranslationKeyboardTarget(targetLanguages.get(i));
            } else {
                AdvanceGramConfig.setTranslationTarget(targetLanguages.get(i));
            }
            callback.run();
        }, resourcesProvider);
    }
    ...
    public static void showTranslationProviderSelector(Context context, MessagesStorage.BooleanCallback callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        PopupHelper.show(names, LocaleController.getString("TranslationProvider", R.string.TranslationProvider), index, context, i -> {
            ...
            String targetLanguage = translator.getTargetLanguage(AdvanceGramConfig.translationTarget);

            if (translator.supportLanguage(targetLanguage)) {
                AdvanceGramConfig.setTranslationProvider(types.get(i));
                ...
                if ("app".equals(AdvanceGramConfig.translationTarget)) {
                    builder.setPositiveButton(LocaleController.getString("UseGoogleTranslate", R.string.UseGoogleTranslate), (dialog, which) -> {
                        AdvanceGramConfig.setTranslationProvider(Translator.PROVIDER_GOOGLE);
                    ...
                    builder.setPositiveButton(LocaleController.getString("ResetLanguage", R.string.ResetLanguage), (dialog, which) -> {
                        AdvanceGramConfig.setTranslationProvider(types.get(i));
                        AdvanceGramConfig.setTranslationTarget("app");
                        if (callback != null) callback.run(false);
                    });
                    builder.setNegativeButton(LocaleController.getString("Cancel", R.string.Cancel), null);
                } else {
    ...
    public static BaseTranslator getCurrentTranslator() {
        return getTranslator(AdvanceGramConfig.translationProvider);
    }
    ...
    @NotNull
    public Object doTranslate(@NotNull String s, @NotNull String language, @NotNull String query) {
        return null;
    }
