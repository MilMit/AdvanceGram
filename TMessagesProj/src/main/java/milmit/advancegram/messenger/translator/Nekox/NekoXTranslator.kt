package milmit.advancegram.messenger.translator.Nekox
...
import milmit.advancegram.messenger.ui.PopupBuilder
import milmit.advancegram.messenger.cc.CCConverter
import milmit.advancegram.messenger.cc.CCTarget
import milmit.advancegram.messenger.translator.Nekox.source.*
import milmit.advancegram.messenger.utils.UIUtil
import milmit.advancegram.messenger.utils.receive
import milmit.advancegram.messenger.utils.receiveLazy
import java.util.*
...
interface NekoXTranslator {
    ...
    companion object {
        ...
        suspend fun translate(query: String) = translate(
            AdvConfig.translateToLang.String()?.code2Locale
        ...
        suspend fun translate(to: Locale, query: String): String {
            ...
            val provider = AdvConfig.translationProvider.Int()
            when (provider) {
            ...
            if (language == "zh") {
                ...
                if (countryUpperCase == "CN") {
                    return CCConverter.get(CCTarget.SP).convert(result as String)
        ...
        @JvmStatic
        @JvmOverloads
        fun translate(to: Locale = AdvConfig.translateToLang.String()?.code2Locale
