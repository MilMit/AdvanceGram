/*
 * This is the source code of AdvanceGram for Android v. 1.4.x.
 * It is licensed under GNU GPL v. 2 or later.
 * You should have received a copy of the license in this archive (see LICENSE).
 *
 * Copyright Laky64, 2021-2022.
 */
package milmit.advancegram.messenger.camera;
...
public class CameraXUtils {
    ...
    public static void loadSuggestedResolution() {
        ...
        getAvailableVideoSizes().values().stream()
                ...
                .ifPresent(height -> {
                    ...
                    if (AdvanceGramConfig.cameraResolution == -1 || AdvanceGramConfig.cameraResolution > maxRes || AdvanceGramConfig.cameraResolution < lowRes) {
                        AdvanceGramConfig.saveCameraResolution(height);
                    }
                });
    }
    ...
    public static Size getPreviewBestSize() {
        ...
        return getAvailableVideoSizes().values().stream()
                .filter(size -> size.getHeight() <= AdvanceGramConfig.cameraResolution && size.getHeight() < suggestedRes)
                .findFirst()
                .orElse(new Size(0, 0));
    }
    ...
    public static Quality getVideoQuality() {
        return getAvailableVideoSizes().entrySet().stream()
                .filter(entry -> entry.getValue().getHeight() == AdvanceGramConfig.cameraResolution)
    ...
    private static int getSuggestedResolution(boolean isPreview) {
        ...
        switch (SharedConfig.getDevicePerformanceClass()) {
            ...
            default:
                suggestedRes = AdvanceGramConfig.useCameraXOptimizedMode && isPreview ? 1080 : 2160;
                break;
        }
        return suggestedRes;
    }
    ...
    public int getCurrentResolution() {
        return getAvailableVideoSizes().values().stream()
                .mapToInt(Size::getHeight)
                .filter(size -> size == AdvanceGramConfig.cameraResolution)
                .findFirst()
                .orElse(-1);
    }
