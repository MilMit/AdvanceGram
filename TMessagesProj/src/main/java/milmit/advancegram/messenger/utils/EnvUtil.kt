...
object EnvUtil {
    ...
    val availableDirectories
        get() = LinkedList<File>().apply {
            ...
            if (Build.VERSION.SDK_INT < 30) {
                add(Environment.getExternalStoragePublicDirectory("Advgram"))
            }
    ...
    fun getTelegramPath(): File {
        ...
        if (AdvConfig.cachePath.String() == "") {
            // https://github.com/NekoX-Dev/NekoX/issues/284
            AdvConfig.cachePath.setConfigString(availableDirectories[2]);
        }
        var telegramPath = File(AdvConfig.cachePath.String())
