...
import milmit.advancegram.messenger.proxy.ShadowsocksLoader
import milmit.advancegram.messenger.ui.BottomBuilder
import milmit.advancegram.messenger.utils.AlertUtil.showToast
...
object ProxyUtil {
        ...
        text.split('\n').map { it.split(" ") }.forEach {
            ...
            it.forEach { line ->
                ...
                if (line.startsWith("tg://proxy") ||
                    ...
                    line.startsWith(V2RayConfig.VMESS_PROTOCOL) ||
                    line.startsWith(V2RayConfig.VMESS1_PROTOCOL) ||
                    line.startsWith(V2RayConfig.SS_PROTOCOL) ||
                    line.startsWith(V2RayConfig.SSR_PROTOCOL) ||
                    line.startsWith(V2RayConfig.WS_PROTOCOL) ||
                    line.startsWith(V2RayConfig.WSS_PROTOCOL) ||
                    line.startsWith(V2RayConfig.TROJAN_PROTOCOL) /*||
    ...
    fun importProxy(ctx: Context, link: String): Boolean {
        ...
        runCatching {
            ...
            if (link.startsWith(V2RayConfig.VMESS_PROTOCOL) || link.startsWith(V2RayConfig.VMESS1_PROTOCOL)) {

                AndroidUtilities.showVmessAlert(ctx, SharedConfig.VmessProxy(link))

            } else if (link.startsWith(V2RayConfig.TROJAN_PROTOCOL)) {

                AndroidUtilities.showTrojanAlert(ctx, SharedConfig.VmessProxy(link))

            } else if (link.startsWith(V2RayConfig.SS_PROTOCOL)) {

                AndroidUtilities.showShadowsocksAlert(ctx, SharedConfig.ShadowsocksProxy(link))

            } else if (link.startsWith(V2RayConfig.SSR_PROTOCOL)) {

                AndroidUtilities.showShadowsocksRAlert(ctx, SharedConfig.ShadowsocksRProxy(link))

            } else if (link.startsWith(V2RayConfig.WS_PROTOCOL) || link.startsWith(V2RayConfig.WSS_PROTOCOL)) {
    ...
    fun showLinkAlert(ctx: Activity, text: String, tryInternal: Boolean = true) {
        ...
        builder.addItems(arrayOf(
            ...
            R.drawable.web_access,
            R.drawable.ic_ab_copy,
