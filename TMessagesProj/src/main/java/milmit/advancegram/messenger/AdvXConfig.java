...
import milmit.advancegram.messenger.database.NitritesKt;
...
//            1137038259, // NekoX APKs
    ...
    public static SharedPreferences preferences = NitritesKt.openMainSharedPreference("adv_config");
...
//    public static long nextUpdateCheck = preferences.getLong("nextUpdateCheckTimestamp", 0);
    ...
    public static String getOpenPGPAppName() {
        if (StrUtil.isNotBlank(AdvConfig.openPGPApp.String())) {
            try {
                PackageManager manager = ApplicationLoader.applicationContext.getPackageManager();
                ApplicationInfo info = manager.getApplicationInfo(AdvConfig.openPGPApp.String(), PackageManager.GET_META_DATA);
                return (String) manager.getApplicationLabel(info);
            } catch (PackageManager.NameNotFoundException e) {
                AdvConfig.openPGPApp.setConfigString("");
            }
        }
    ...
    public static void setChannelAlias(long channelID, String name) {
        preferences.edit().putString(AdvConfig.channelAliasPrefix + channelID, name).apply();
    }

    public static void emptyChannelAlias(long channelID) {
        preferences.edit().remove(AdvConfig.channelAliasPrefix + channelID).apply();
    }

    public static String getChannelAlias(long channelID) {
        return preferences.getString(AdvConfig.channelAliasPrefix + channelID, null);
    }
}
