    ...
    private void updateIconsVisibility() {
        ...
        for (int i = 0; i < availableIcons.size(); i++) {
            ...
            if (icon.hidden && !LauncherIconController.isEnabled(icon)) {
                if (icon == LauncherIconController.LauncherIcon.TURBO && AdvanceGramConfig.unlockedSecretIcon == -1) {
                    continue;
                } else if (icon == LauncherIconController.LauncherIcon.NEON && AdvanceGramConfig.unlockedChupa) {
                    continue;
                }
                availableIcons.remove(i);
                i--;
